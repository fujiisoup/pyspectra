...
def level(molecule, state, v, J):
    ...
    if state not in constants["state"]:
        raise ValueError(
            "{} is not available. Available states are {}".format(
                state, constants["state"].values
            )
        )
    constants = constants.sel(state=state).fillna(0)
    Evib = (
        constants["we"].item() * (v + 0.5)
        - constants["wexe"].item() * (v + 0.5) ** 2
        + constants["weye"].item() * (v + 0.5) ** 3
    )
    Erot = (
        (
            constants["Be"].item()
            - constants["alpha_e"].item() * (v + 0.5)
            + constants["gamma_e"].item() * (v + 0.5) ** 2
        )
        * J
        * (J + 1)
    ...
    return units.cm_to_eV(constants["Te"].item() + Evib + Erot)
...
def _level_OH_X2(v, J, parity):
    ...
    Ham[..., 0, 0] = (
        ...
        * (
            z - 2 * parity * (J + 0.5)
        )  # <-- this part might be a typo in the original paper
    )
    Ham[..., 1, 1] = (
        Te
        + 0.5 * A
